...
@WebServlet("/TwinsAppart")
public class TwinsAppart extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		this.getServletContext().getRequestDispatcher("/WEB-INF/Twins.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(recue.equals("Reserver")) {
			try {
				...
				Tarif trf = new Tarif();
				String type = "TWINS APPART";//request.getParameter("type")
				trf.setTypeA(type);
				trf.setPrixJ();
				//try {
			...
			request.setAttribute("donneAppart", acces.sortieAppart("TWINS APPART"));
