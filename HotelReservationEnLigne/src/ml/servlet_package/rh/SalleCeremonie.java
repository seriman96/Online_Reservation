...
@WebServlet("/SalleCeremonie")
public class SalleCeremonie extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		this.getServletContext().getRequestDispatcher("/WEB-INF/Ceremonie.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(recue.equals("Reserver")) {
			try {
				...
				Tarif trf = new Tarif();
				String type = "salle de cérémonie";//request.getParameter("type")
				trf.setTypeS(type);
				trf.setPrixJS();
				//try {
					trf.setDebutR(request.getParameter("debut"));
				/*} catch (BeansException e4) {
					request.setAttribute("erreurBeans", e4.getMessage());
				}*/
				//try {
					trf.setFinR(request.getParameter("fin"));
				/*} catch (BeansException e3) {
					request.setAttribute("erreurBeans", e3.getMessage());
				}*/
				trf.setSejour();
				...
				/*} catch (DaoException e2) {
					request.setAttribute("erreurDao", e2.getMessage());
				}*/
				//try {
					acces.ajoutTar(trf);
				/*} catch (DaoException e1) {
					request.setAttribute("erreurDao", e1.getMessage());
				}*/
				//try {
			...
			request.setAttribute("donneAppart", acces.sortieSalle("salle de cérémonie"));
