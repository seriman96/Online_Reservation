...
@WebServlet("/ReservationRHS")
public class ReservationRHS extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		this.getServletContext().getRequestDispatcher("/WEB-INF/Formulaire_Salle.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(recue.equals("Reserver")) {
			try {
				...
				Tarif trf = new Tarif();
				trf.setTypeS(request.getParameter("type"));
				trf.setPrixJS();
				//try {
					trf.setDebutR(request.getParameter("debut"));
				/*} catch (BeansException e) {
					request.setAttribute("erreurBeans", e.getMessage());
				}*/
				//try {
			...
			request.setAttribute("donneAppart", acces.sortieSalle(request.getParameter("type")));
